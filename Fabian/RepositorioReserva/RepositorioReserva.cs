...
namespace Inmobiliaria_.Net_Core.Models
{
    public class RepositorioReserva : RepositorioBase, IRepositorioReserva
    {
        ...
        {
            const string sql = @"INSERT INTO reserva (IdInmueble, IdInquilino, FechaDeEntrada, FechaDeSalida, Estado, MontoDiario, MontoTotal, UsuarioCreacionId)
                VALUES (@inmueble, @inquilino, @entrada, @salida, 'Activo', @montoDiario, @montoTotal, @usuario);";
            
            using var connection = new MySqlConnection(connectionString); 
            using var command = new MySqlCommand(sql, connection);
            
            command.Parameters.AddWithValue("@inmueble", p.IdInmueble); 
            command.Parameters.AddWithValue("@inquilino", p.IdInquilino); 
            command.Parameters.AddWithValue("@entrada", p.FechaDeEntrada); 
            command.Parameters.AddWithValue("@salida", p.FechaDeSalida); 
            command.Parameters.AddWithValue("@montoDiario", p.MontoDiario); 
            command.Parameters.AddWithValue("@montoTotal", p.MontoTotal);
            command.Parameters.AddWithValue("@usuario", (object?)p.UsuarioCreacionId ?? DBNull.Value);
            
            connection.Open(); 
            command.ExecuteNonQuery(); 
            p.IdReserva = (int)command.LastInsertedId; 
            return p.IdReserva;
        }
        ...
        { 
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand("UPDATE reserva SET Estado='Cancelado', UsuarioFinalizacionId=@usuario WHERE IdReserva=@id", c); 
            q.Parameters.AddWithValue("@id", id); 
            q.Parameters.AddWithValue("@usuario", usuarioId); 
            c.Open(); 
            return q.ExecuteNonQuery(); 
        }
        ...
        {
            const string sql = "UPDATE reserva SET IdInmueble=@inmueble, IdInquilino=@inquilino, FechaDeEntrada=@entrada, FechaDeSalida=@salida, MontoTotal=@montoTotal, MontoDiario=@montoDiario WHERE IdReserva=@id";
            
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand(sql, c); 
            
            q.Parameters.AddWithValue("@inmueble", p.IdInmueble); 
            q.Parameters.AddWithValue("@inquilino", p.IdInquilino); 
            q.Parameters.AddWithValue("@entrada", p.FechaDeEntrada); 
            q.Parameters.AddWithValue("@salida", p.FechaDeSalida); 
            q.Parameters.AddWithValue("@montoTotal", p.MontoTotal);
            q.Parameters.AddWithValue("@montoDiario", p.MontoDiario);
            q.Parameters.AddWithValue("@id", p.IdReserva); 
        ...
        { 
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand("SELECT COUNT(*) FROM reserva", c); 
            c.Open(); 
        ...
        public IList<Inmueble> VerificarDisponibilidad(DateTime inicioFecha, DateTime finFecha, int cupo = 0)
        {
            ...
            const string sql = @"SELECT i.IdInmueble AS Id, i.Direccion, i.Cupo, i.PrecioPorDia, i.PorcentajeReserva, i.PropietarioId, i.Portada 
                                 FROM inmueble i 
                                 WHERE i.Disponible=1 AND i.Cupo>=@cupo 
                                   AND NOT EXISTS (SELECT 1 FROM reserva r WHERE r.IdInmueble=i.IdInmueble AND r.Estado='Activo' AND r.FechaDeEntrada<@fin AND COALESCE(r.FechaFinEfectiva,r.FechaDeSalida)>@inicio)";
            
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand(sql, c); 
            q.Parameters.AddWithValue("@inicio", inicioFecha); 
            q.Parameters.AddWithValue("@fin", finFecha); 
            q.Parameters.AddWithValue("@cupo", cupo); 
            c.Open(); 
            using var reader = q.ExecuteReader(); 
            while (reader.Read()) 
                result.Add(new Inmueble { 
        ...
        {
            const string sql = @"SELECT COUNT(*) FROM reserva 
                                 WHERE IdInmueble=@inmueble 
                                   AND Estado='Activo' 
                                   AND (@excluir IS NULL OR IdReserva<>@excluir) 
                                   AND FechaDeEntrada<@salida 
                                   AND COALESCE(FechaFinEfectiva, FechaDeSalida)>@entrada";
            
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand(sql, c); 
            
            q.Parameters.AddWithValue("@inmueble", idInmueble); 
            q.Parameters.AddWithValue("@entrada", entrada); 
            q.Parameters.AddWithValue("@salida", salida); 
            q.Parameters.AddWithValue("@excluir", (object?)idReservaExcluir ?? DBNull.Value); 
        ...
        public decimal CalcularMulta(int idReserva, DateTime finFecha)
        {
            ...
            decimal diasTotales = (decimal)(reserva.FechaDeSalida.Date - reserva.FechaDeEntrada.Date).TotalDays;
            decimal diasTranscurridos = (decimal)(finFecha.Date - reserva.FechaDeEntrada.Date).TotalDays;
            decimal diasRestantes = diasTotales - diasTranscurridos;

            if (diasRestantes <= 0)
                return 0;

            decimal porcentajeMulta = diasTranscurridos < diasTotales / 2m ? 0.50m : 0.25m;
            decimal alquilerRestante = diasRestantes * reserva.MontoDiario;

            return alquilerRestante * porcentajeMulta;
        }

        public bool TerminarReservaAnticipada(int idReserva, DateTime nuevoFinFecha, int usuarioFinalizacionId)
        {
            const string sql = @"
                UPDATE reserva
        ...
        {
            using var c = new MySqlConnection(connectionString); 
            using var q = new MySqlCommand("UPDATE reserva SET FechaFinEfectiva=@fin, Estado='Finalizada' WHERE IdReserva=@id", c); 
            q.Parameters.AddWithValue("@fin", nuevoFinFecha); 
            q.Parameters.AddWithValue("@id", idReserva); 
            c.Open(); 
            return q.ExecuteNonQuery() > 0;
        ...
        public Reserva RenovarReserva(int idReserva, DateTime nuevoFinFecha, decimal nuevoPrecio)
        {
            ...
            if (nuevoFinFecha <= original.FechaDeSalida || ExisteSolapamiento(original.IdInmueble, original.FechaDeSalida, nuevoFinFecha)) 
                throw new InvalidOperationException("Las fechas seleccionadas no son válidas o están ocupadas.");
            
            var nueva = new Reserva 
            { 
                IdInmueble = original.IdInmueble, 
                IdInquilino = original.IdInquilino, 
                FechaDeEntrada = original.FechaDeSalida, 
                FechaDeSalida = nuevoFinFecha, 
                MontoDiario = nuevoPrecio > 0 ? nuevoPrecio : original.MontoDiario, 
                MontoTotal = (int)((nuevoFinFecha.Date - original.FechaDeSalida.Date).Days * (nuevoPrecio > 0 ? nuevoPrecio : original.MontoDiario)),
